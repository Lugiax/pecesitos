...
def extractor_datos(grab_path, secc_path, args):
    pez = 'piraña' if 'pirana' in args.data_dir else 'cirujano'
    ...
    data = pd.read_csv(grab_path, header=0)
    ...
    if not os.path.isfile(secc_path) or args.graficos:
        fig, ax = plt.subplots()
        ax2 = ax.twinx()
        ...
        plt.title(os.path.basename(args.data_dir))
        plt.show()

    if not os.path.isfile(secc_path):
        ...
        with open(secc_path, 'w') as f:
            f.writelines('\n'.join(lines))
        print('Archivo creado')

    secciones = []
    with open(secc_path, 'r') as f:
        lineas = f.readlines()
        for l in lineas:
            _, inic_fin = l.split(':')
            if 'NA' in inic_fin:
    ...
    for i, fila in enumerate(coordenadas_pos):
        ...
        for j, coord in enumerate(fila):
            ...
            por_profundidades[i][0].append(dist.tolist())
            por_profundidades[i][1].append(e_l.tolist())
            por_profundidades[i][2].append(e_d.tolist())
    ...
    return np.array(calculos), por_profundidades
...
if __name__=='__main__':
    ...
    grab_path = os.path.join(args.data_dir, 'grabación.csv')
    secc_path = os.path.join(args.data_dir, 'secciones.txt')

    calculos, por_profundidades = extractor_datos(grab_path, secc_path, args)
    calculos = np.array(calculos)
    n_Ps = calculos.shape[0]
    #Gráfico de longitudes por profundidades-------------------------------------------------------------------------------------------
    #colors = ['#1D3461', '#1F487E',  
    #          '#376996', '#6290C8',
    #          '#7296C2', '#829CBC']
    fig, ax = plt.subplots(figsize=(7,4))
    bp = ax.boxplot([flatten(p[0]) for p in por_profundidades],
    ...
    errores = np.array([[np.mean(flatten(p[1])), np.mean(flatten(p[2]))] for p in por_profundidades])
    fig, ax = plt.subplots(figsize=(7,4))
